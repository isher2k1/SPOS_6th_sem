...
public class ls
{
  ...
  private static void print( String name , Stat stat )
  {
    ...
    // append the uid in a field of 15
    ...
    // append the gid in a field of 20
   	t = Integer.toString( stat.getGid() ) ;
    for( int i = 0 ; i < 20 - t.length() ; i ++ )
    	s.append( ' ' ) ;
    s.append( t ) ;
    s.append( ' ' ) ;

    // append the 9 low-order bits of mode as a 3-digit octal number in a field of 25
    t = Integer.toString( stat.getMode() ) ;
    for( int i = 0; i < 20 - t.length() ; i ++ )
    	s.append( ' ' ) ;
    s.append( t ) ;
    s.append( ' ' ) ;
